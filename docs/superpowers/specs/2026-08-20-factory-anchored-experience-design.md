...
| **StackGen demo videos** | Product screenshots as the proof layer | Anything else |
...
| D2 | Proof layer is real product screenshots extracted from StackGen demo videos. |
...
`Reveal` and `MotionProvider` remain the hooks and stay inert in this phase. Motion is specified but not implemented here, so that Wave 2 needs no refactor. Only `transform`, `opacity`, `filter` and `clip-path` may ever be animated. No animation may shift layout. `prefers-reduced-motion` must preserve state change and hierarchy rather than killing all motion globally.
...
Process: `yt-dlp` to fetch at 1080p, `ffmpeg` to extract candidate frames, select the frames that show a complete meaningful state, redact, then commit to `web/public/product/`.

**Redaction is mandatory.** Observed in raw frames: an AWS account ID (`180217099948`) and live bucket names including `aiden-eval-reports`, `prathvi-aiden-infra-backend-bucket` and `stackgen-terraform-state-bucket-a928b57`. Every frame must be reviewed for account IDs, ARNs, bucket and repository names, internal hostnames, email addresses and customer identifiers before it is committed. Replace with plausible neutral equivalents rather than blurring, so the screenshot still reads as real.

Screenshots render inside `ProductFrame`: a dark bezel on the cream ground with an iridescent glow, mirroring the deck's treatment on slide 5. Each requires descriptive alt text stating what the surface shows, not "product screenshot".
...
| `ProductFrame` | Dark bezel plus iridescent glow around a screenshot |
| `Marquee` | Logo and model scrollers, pauses on `prefers-reduced-motion` |
...
6. Every committed product frame has a redaction sign-off recorded in the deviation log
