...
| 11 | Integration tool logos sourced from Simple Icons CDN (`simple-icons@v11`) vs missing Pencil export nodes | Asset / Parity | Recorded Pencil component IDs (`j8jzF`, `HoxfJ`, …) all MISSING from current `Stack_Linear.pen`; no substitute from elsewhere per T8 brief | `K1zfG` / `/` | T8 report |
